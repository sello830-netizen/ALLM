...
الـtest لا يستخدم لاختيار النموذج. سجل التدريب الآن يتضمن `eval_oov_rate` و`eval_known_perplexity` حتى نفصل ضعف التغطية المعجمية عن خسارة الكلمات المعروفة. الخطوة التالية هي بناء views محدودة ومتحققًا منها، ثم مقارنة original-only وmulti-view تحت نفس token/step budget.
